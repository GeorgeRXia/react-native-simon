...
class RoundsMatchingMultiplayer extends Component {
  ...
  onScoreReceived = (event) => {
    ...
    if (receivedEvent.gameStatus === "done") {
      ...
      this.setState({
        gameSetter: receivedEvent.gameStatus
      })
  ...
  renderTiles() {
    ...
    for (var row = 0; row < SIZE; row++) {
      for (var col = 0; col < SIZE; col++) {
        ...
        result.push(this.renderTile(i++, position, {
  ...
  renderTile(id, position, bgColor, litBgColor) {
    ...
    return ( < TouchableOpacity onPress = {
      ...
      <
      View style = {
        [styles.tile, position, this.state.lit == id ? litBgColor : bgColor]
      } > < Text style = {
        styles.letter
      } > {
        id
