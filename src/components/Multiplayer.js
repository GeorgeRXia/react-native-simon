...
class Multiplayer extends Component {
  ...
  onScoreReceived = (event) => {
    ...
    if(receivedEvent.gameStatus === "done") {
      this.setState({history: receivedEvent.score,opponentsGameStatus: receivedEvent.gameStatus})

    }else {
      this.setState({history: event.data})
  ...
  onSendScore = (status) => {
    ...
    if(status === "done") {
      this.setState({ownGameStatus: "done" })
  ...
  animateSequence(newSequence) {
    ...
    for(let i = 0; i < newSequence.length; i++) {
      setTimeout(function(){
        setTimeout(function(){
          that.setState({lit: newSequence[i]})}, 800);

        if(newSequence[i] === newSequence[i-1]){
          setTimeout(function(){that.setState({lit:0})},990);

        }else{
          setTimeout(function(){that.setState({lit: 0})},1000);
        }
      }, i * 1000);
